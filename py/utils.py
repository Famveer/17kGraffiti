    ...
    def __getitem__(self, idx):
        ...
        if self.transforms is not None:
            sample = {
                'image': img,
                'bboxes': target['boxes'],
                'labels': labels.tolist()  # Convert tensor to list for albumentations
            }
            sample = self.transforms(**sample)
            img = sample['image']
            target['boxes'] = torch.stack(tuple(map(torch.tensor, zip(*sample['bboxes'])))).permute(1, 0)
            target['boxes'] = target['boxes'].float()
            target['labels'] = torch.tensor(sample['labels'], dtype=torch.int64)  # Convert labels back to tensor
